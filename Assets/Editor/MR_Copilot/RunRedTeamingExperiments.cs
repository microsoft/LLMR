...
public class RunRedTeamingExperiments : EditorWindow
{
    //public int num_exp_to_run = 1;
    public string exp_name = "test";
    public string file_name = "red_teaming_test";
    public string chat_manager_name = "ChatManager";
    public ExperimentType exp_type = ExperimentType.Single_Prompts;
    public bool use_filter = true;
    public bool use_scene_analyzer = false;
    public bool use_skill_library = false;
    public bool use_inspector = false;
    public bool use_demonstrations = true;
    public ChatCompilationManagerModularFiltered chat_manager;
    ...
    public string exp_csv_dir_rel = Path.Combine("Experiments", "results");

    public string exp_prompt_dir;
    public string exp_result_dir;
    public string exp_context_dir;
    public string exp_recording_dir;
    public string exp_csv_dir;

    public int delay_time_entering_play_mode = 2500;
    public int delay_time_between_prompts = 2500;
    ...
    [MenuItem("Experiments/RunRedTeaming")]
    public static void ShowWindow()
    {
        // Get or create the window instance
        var window = GetWindow<RunRedTeamingExperiments>();
        // Set the window title
        window.titleContent = new GUIContent("Run Red Teaming Experiments");
        // Show the window
        window.Show();
    }
    ...
    void ManageExperimentDirs()
    {
        ...
        exp_csv_dir = Path.Combine(Application.dataPath, exp_csv_dir_rel);
        ...
        CreateDirIfNotExist(exp_csv_dir);
    }
    ...
    async Task RunAllExperiments()
    {
        ...
        // but first reset it in case of a previous run
        exp_result_dir_rel = Path.Combine("Experiments", "results");
    ...
    async Task RunExperiment()
    {
        ...
        chat_manager = GameObject.Find(chat_manager_name).GetComponent<ChatCompilationManagerModularFiltered>();
        chat_manager.use_filter = use_filter;
    ...
    async Task RunSinglePromptExperiments(string prompt_str)
    {
        ...
        // Initialize the CSV file in the 
        Debug.Log("Initializing CSV");
        string exp_result_path = Path.Combine(exp_csv_dir, "redteaming_results.csv");
        SetupCSVFile(exp_result_path, "Experiment Filename,Type,Prompt,Output,Errors\n");
        ...
        for (int i = 0; i < prompts.Count; i++)
        {
            ...
            await Task.Delay(delay_time_entering_play_mode);

            DateTime exampleStartTime = DateTime.Now;
            ...
            await Task.Delay(delay_time_between_prompts);
            ...
            string build_output = chat_manager.builder.output;
            string cleaned_output = new string(build_output.Where(c => !char.IsControl(c)).ToArray());
            cleaned_output = cleaned_output.Replace(@",", " ");
            // record the result
            File.AppendAllText(results_path, "prompt: " + prompt + "; Output: " + cleaned_output + "; Errors: " + errs +  '\n');
            ...
            WriteLineToCSV(exp_result_path, $"{file_name},{exp_name},{prompt},{cleaned_output},{errs}\n");
    ...
    async Task RunSequentialPromptExperiments(string prompt_str)
    {
        ...
        // Initialize the CSV file in the 
        Debug.Log("Initializing CSV");
        string exp_result_path = Path.Combine(exp_csv_dir, "results_sequential.csv");
        SetupCSVFile(exp_result_path, "Experiment Filename,Type,Percentage of errors generated," +
            "Average percentage of passed prompts,Percentage of sequential prompts completely fulfilled,Average Time,Average Time for passed sequences\n");

        DateTime startTime = DateTime.Now;
        TimeSpan totalTime = TimeSpan.Zero;
        TimeSpan totalTimePassedOnes = TimeSpan.Zero;
        ...
        for (int i = 0; i < prompts_seq.Count; i++)
        {
            ...
            for (int j = 0; j < prompt_seq.Count; j++)
            {
                ...
                // clear errors
                chat_manager.ClearRecordedErrorMessages();
            ...
            TimeSpan timeToOutcome = DateTime.Now - exampleStartTime;
            totalTime += timeToOutcome;
            ...
            {
                accomplished += 1;
                totalTimePassedOnes += timeToOutcome;
            }
            ...
            File.AppendAllText(results_path, "prompt: " + String.Join("; ", prompt_seq.ToArray()) + "; " +
                "Result: " + outcome + "; Percentage accomplished: " + percentage_passed + "; Errors: " + errs + "; Time: " + timeToOutcome + '\n');
        }
        ...
        File.AppendAllText(results_path, "Average time for the sequence: " + totalTime / total_prompt_seq_num + '\n');
        File.AppendAllText(results_path, "Average time for the sequences that fully passed: " + totalTimePassedOnes / accomplished + '\n');

        WriteLineToCSV(exp_result_path, $"{file_name},{exp_name},{num_error / total_prompt_num}," +
            $"{percentage_accomplished / total_prompt_seq_num},{accomplished / total_prompt_seq_num},{totalTime / total_prompt_seq_num}" +
            $"{totalTimePassedOnes / accomplished}\n");
    ...
    void SetupCSVFile(string path, string header)
    {
        if (!File.Exists(path))
        {
    ...
    void WriteLineToCSV(string path, string line)
    {
    ...
    async void OnGUI()
    {
        ...
        delay_time_between_prompts = EditorGUILayout.IntField("Delay time between prompts (ms)", delay_time_between_prompts);
        ...
        use_filter = EditorGUILayout.Toggle("Use Content Filter", use_filter);
