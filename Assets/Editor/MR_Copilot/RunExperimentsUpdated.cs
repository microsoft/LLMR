...
public class RunExperimentsUpdated : EditorWindow
{
    ...
    [MenuItem("Experiments/RunUpdated")]
    public static void ShowWindow()
    {
        // Get or create the window instance
        var window = GetWindow<RunExperimentsUpdated>();
        // Set the window title
        window.titleContent = new GUIContent("Run Experiments With Timing");
        // Show the window
        window.Show();
    }
    ...
    async Task RunSinglePromptExperiments(string prompt_str)
    {
        ...
        string exp_result_path = Path.Combine(exp_csv_dir, "results.csv");
        SetupCSVFile(exp_result_path, "Experiment Filename,Type,Average Error Percentage,Average Time\n");
        ...
        for (int i = 0; i < prompts.Count; i++)
        {
            ...
            Debug.Log("This prompt took " + timeToOutcome +" of time");
            // record the result
            File.AppendAllText(results_path, "prompt: " + prompt + "; Result: " + outcome + "; Errors: " + errs + "; Time: "+ timeToOutcome + '\n');
        ...
        float averageErrorPercentage = num_error / total_num;

        // record the result
        File.AppendAllText(results_path, "Percentage of errors generated: " + averageErrorPercentage);


        TimeSpan averageTime = totalTime / total_num;

        WriteLineToCSV(exp_result_path, $"{file_name},{exp_name},{averageErrorPercentage},{averageTime}\n");
        //m_RecorderController.StopRecording();
        //m_IsRecording = false;
    }
