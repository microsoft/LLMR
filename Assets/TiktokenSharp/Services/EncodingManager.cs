...
namespace TiktokenSharp.Services
{
    internal class EncodingManager
    {
        ...
        private Dictionary<byte[], int> LoadTikTokenBpeFromResource()
        {
            // assumes the BPE tokenizer is stored at Assets/Resources/GPT_Tokenizer/cl100k_base

            var bpeDict = new Dictionary<byte[], int>(new ByteArrayComparer());

            try
            {
                var lines = Resources.Load<TextAsset>(PBEFileDirectory).ToString().Split('\n');
            ...
            {
                throw new InvalidOperationException($"Failed to load TikTokenBpe: {ex.Message}", ex);
            }
        ...
        private async Task<EncodingSettingModel> cl100k_base()
        {
            ...
            var mergeable_ranks = LoadTikTokenBpeFromResource();
